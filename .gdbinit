...
# Given a cluster-core, returns the PC of the next instruction to be decoded
...
# Given a cluster-core-warp, returns the current inst
define get_current_inst_issue
	if GPGPU_Context()->the_gpgpusim->g_the_gpu->m_cluster[$arg0]->m_core[$arg1]->m_kernel
		print *(GPGPU_Context()->the_gpgpusim->g_the_gpu->m_cluster[$arg0]->m_core[$arg1]->m_kernel->m_kernel_entry->get_instruction(GPGPU_Context()->the_gpgpusim->g_the_gpu->m_cluster[$arg0]->m_core[$arg1]->m_warp[$arg2]->m_next_pc))
	else
		print -1
	end
end

# Given a cluster-core, prints the current kernel
define get_current_kernel
	print *(GPGPU_Context()->the_gpgpusim->g_the_gpu->m_cluster[$arg0]->m_core[$arg1]->m_kernel)
end
